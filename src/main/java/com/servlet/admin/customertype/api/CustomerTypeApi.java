...
public class CustomerTypeApi {
	...
	@DeleteMapping("{id}")
	ResponseEntity<Response> deleteCustomerType(@PathVariable long id, @RequestHeader(ConstansKey.AUTH) String authorization) {
		Response response = securityService.response(ConstansPermission.DELETE_CUSTOMERTYPE,id,authorization);
		return ResponseEntity.status(response.getHttpcode()).contentType(MediaType.APPLICATION_JSON).body(response);
	}
