...
public interface UserMobileRepo extends JpaRepository<UserMobile, Long>{
	...
	@Query(value =" select * from m_user_mobile "
			+ " where username = :user and imei = :imeihp and isdelete = false ",nativeQuery = true)
	public List<UserMobile> getUserLoginByUsername(@Param("user") String user,@Param("imeihp") String imeihp);
