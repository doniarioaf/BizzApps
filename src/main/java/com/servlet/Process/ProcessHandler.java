		...
		if(auth.getTypelogin().equals(ConstansKey.TYPE_WEB)) {
			if(codepermission.equals(ConstansPermission.READ_ROLE)) {
				...
				if(type.equals("ALL")) {
					...
					Long date = (Long) param.get("date");
					val.setData(purchaseReceiveService.searchDataByVendor(auth.getIdcompany(), auth.getIdbranch(),idvendor,date));
...
//				}
			...
			else if(codepermission.equals(ConstansPermission.READ_REPORT_PIUTANG)) {
				HashMap<String, Object> param = (HashMap<String, Object>) data;
				String type = (String) param.get("type");
				if(type.equals("REPORTPIUTANG")) {
					ParamReportPiutang body = (ParamReportPiutang) param.get("body");
					val.setData(reportService.reportPiutang(auth.getIdcompany(), auth.getIdbranch(),body).getWorkbook());
				}else if(type.equals("REPORTPIUTANG_TEMPLATE")) {
					val.setData(reportService.reportTemplateReportPiutang(auth.getIdcompany(), auth.getIdbranch()));
				}
			}

			else if(codepermission.equals(ConstansPermission.READ_REPORT_PENJUALAN)) {
				HashMap<String, Object> param = (HashMap<String, Object>) data;
				String type = (String) param.get("type");
				if(type.equals("REPORTPENJUALAN")) {
					ParamReportPenjualan body = (ParamReportPenjualan) param.get("body");
					val.setData(reportService.reportPenjualan(auth.getIdcompany(), auth.getIdbranch(),body).getWorkbook());
				}else if(type.equals("REPORTPENJUALAN_TEMPLATE")) {
					val.setData(reportService.reportTemplateReportPiutang(auth.getIdcompany(), auth.getIdbranch()));
				}
			}
				...
				HashMap<String, Object> param = (HashMap<String, Object>) data;
				String type = (String) param.get("type");
				if(type.equals("REPORTKARTUSTOCK")) {
					ParamReportKartuStock body = (ParamReportKartuStock) param.get("body");
