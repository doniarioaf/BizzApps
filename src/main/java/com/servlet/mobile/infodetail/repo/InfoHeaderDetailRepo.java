...
public interface InfoHeaderDetailRepo extends JpaRepository<InfoHeaderDetail, Long>{
	...
	@Transactional
	@Modifying
	@Query(value ="update m_info_header_detail set isdelete = true where idinfoheader = :idinfoheader",nativeQuery = true)
	void deleteAllInfoDetailByInfoHeader(@Param("idinfoheader") long idinfoheader);
}
